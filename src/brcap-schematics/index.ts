...
export function brcapSchematics(_options: any): Rule {
	return (tree: Tree, _context: SchematicContext) => {
		...
		const chainedRule = chain([ templateRule, updateModuleRule, installBrCapViewComponentRule ]);
...
function updateRootModule(_options: any, workspace: any): Rule {
	return (tree: Tree, _context: SchematicContext): Tree => {
		...
		const rootModulePath =
			`${project.root}/` + `${project.sourceRoot}/` + `${project.prefix}/` + `${project.prefix}.module.ts`;

		const importContent =
			`import { ${exportedModuleName}Module } ` + `from './${modulePath}/${moduleName}/${moduleName}.module';`;
						...
						if (property.getFullText().includes('dependencies')) {
							property.forEachChild((dependency) => {
								if (dependency.getFullText().includes(materialDepName)) {
									_context.logger.info('✅️ BrCapViewComponent already installed');
